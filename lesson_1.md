...
за этим именем стоит (например, за под именем `int` скрывается класс для создания целых чисел). Если имя неизвестно инерпретатору, т.е. он не знает, что конкретно за ним стоит, вы получите ошибку `NameError`.
- если некий набор букв окружен кавычками, то интерпретатор будет относится к нему как к обычному тексту.
